...
from tools.database import ProductMap, get_product_dict
...
class ProductManager:
    ...
    def main_menu(self) -> None:
        ...
        welcome_l = Label(
            self.menu_frame,
            text="""Seleccione una opcion para manejar la
informacion de los productos utilizada
en las operaciones de venta y reporte.""",
        ...
        manage = Button(
            self.menu_frame,
            text="Manejar un producto que ya existe",
            fg="black",
            bg="whitesmoke",
            font=("Calibri", "14", "bold"),
            command=lambda: self.move_to_option("home", "manage"),
        ...
        delete = Button(
            ...
            bg="whitesmoke",
            font=("Calibri", "14", "bold"),
            command=lambda: self.move_to_option("home", "delete"),
        ).grid(row=2, column=0, sticky="ew")
        # option to create
        create = Button(
            self.menu_frame,
            text="Agregar un producto a la lista",
            fg="black",
            bg="whitesmoke",
            font=("Calibri", "14", "bold"),
            command=lambda: self.move_to_option("home", "create"),
        ).grid(row=3, column=0, sticky="ew")
        # option to show
        show = Button(
            self.menu_frame,
            text="Ver la lista de productos",
            fg="black",
            bg="whitesmoke",
            font=("Calibri", "14", "bold"),
            command=lambda: self.move_to_option("home", "show"),
    ...
    def manage(self) -> None:
        ...
        def manage_product():
            try:
                target, m_value, o_value = (
                    self.product_dict.product_index[self.manage_selection.get() - 1],
                    self.manage_new_value.get(),
                    self.manage_odo_value.get(),
                )
...
Verifique sus entradas e intente de nuevo.""",
        ...
        l1 = Label(
            self.manage_frame,
            text="2. Introduzca el valor de odometro:",
            bg="whitesmoke",
            fg="black",
            font=("Calibri", "13", "bold"),
        ).grid(row=1, column=0, sticky="ew")
        new_odo = Entry(
            self.manage_frame, textvariable=self.manage_odo_value, width=40
        ).grid(row=1, column=1, sticky="ew")
        l2 = Label(
            self.manage_frame,
            text="2. Introduzca un valor para reemplazar (pesos mexicanos):",
            bg="whitesmoke",
            fg="black",
            font=("Calibri", "13", "bold"),
        ).grid(row=2, column=0, sticky="ew")
        new_value = Entry(
            self.manage_frame, textvariable=self.manage_new_value, width=40
        ...
        cancel = Button(
            ...
            bg="red",
            fg="whitesmoke",
            command=lambda: self.move_to_option("manage", "home"),
        ).grid(row=3, column=0, sticky="ew")
        move_it = Button(
            self.manage_frame,
            text="Modificar",
    ...
    def delete(self) -> None:
        ...
        def delete_item():
            try:
                target = self.product_dict.product_index[
                    self.delete_selection.get() - 1
                ]
                new = {}
...
Verifique sus entradas e intente de nuevo.""",
        ...
        l = Label(
            self.delete_frame,
            text="Seleccione el articulo a eliminar:",
            font=("Calibri", "13", "bold"),
            fg="black",
            bg="whitesmoke",
        ).grid(row=0, column=0, sticky="ew")
        entry = get_menubutton(
            self.delete_frame,
            self.delete_options,
        ...
        cancel = Button(
            ...
            bg="red",
            fg="whitesmoke",
            command=lambda: self.move_to_option("delete", "home"),
            font=("Calibri", "13", "bold"),
        ).grid(row=1, column=0, sticky="ew")
        go_for_it = Button(
            self.delete_frame,
    ...
    def create(self) -> None:
        ...
        def create_product():
            try:
                target, money_value, odo_value = (
                    self.create_selection.get(),
                    self.create_new_value.get(),
                    self.create_odo_value.get(),
                )
                if target in self.product_dict.products.keys():
                    raise ValueError(
                        f"El valor '{target}' ya existe. Mejor utilice la funcion de manejo."
                    )
...
Verifique sus entradas e intente de nuevo.""",
        ...
        l = Label(
            self.create_frame,
            text="1. Introduzca el nuevo producto:",
            bg="whitesmoke",
            fg="black",
            font=("Calibri", "13", "bold"),
        ).grid(row=0, column=0, sticky="ew")
        new_opt = Entry(
            self.create_frame, textvariable=self.create_selection, width=40
        ...
        new_odo = Entry(
            self.create_frame, textvariable=self.create_odo_value, width=40
        ).grid(row=1, column=1, sticky="ew")
        l2 = Label(
            self.create_frame,
            text="3. Introduzca un valor para reemplazar (pesos mexicanos):",
            bg="whitesmoke",
            fg="black",
            font=("Calibri", "13", "bold"),
        ).grid(row=2, column=0, sticky="ew")
        new_value = Entry(
            self.create_frame, textvariable=self.create_new_value, width=40
        ).grid(row=2, column=1, sticky="ew")
        cancel = Button(
            self.create_frame,
            text="Cancelar",
            bg="red",
            fg="whitesmoke",
            command=lambda: self.move_to_option("create", "home"),
        ).grid(row=3, column=0, sticky="ew")
        move_it = Button(
            self.create_frame,
            text="Modificar",
    ...
    def finish_message(self, event: str) -> None:
        ...
        messagebox.showinfo(
            ...
            f"""El proceso se ha completado con exito.
    ...
    def move_to_option(self, origin: str, dest: str) -> None:
        ...
        if result == ("home", "show"):
            ...
            messagebox.showwarning(
                ...
                f"""La opcion del menu no fue identificada: {result}.

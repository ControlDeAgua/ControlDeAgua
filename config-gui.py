...
from tools.pathfinders import find_our_file, identify_dir
    ...
    def __init__(self, root: Tk) -> None:
        ...
        if get_admin_pwd() is not None:
            ...
            messagebox.showwarning(
                "Advertencia: Cuenta de Administrador desprotegida",
                """La cuenta del administrador no tiene
...
y privilegios del administrador.""",
    ...
    def ensureAdmin(self, next_page: Optional[Callable] = None) -> Optional[bool]:
        ...
        door_l = Label(
            self.adminport,
            text="Ingrese contraseña del administrador del programa:",
    ...
    def notDefined(self, arg: str) -> None:
        ...
        messagebox.showinfo(
            str(NotImplemented), f"Funcion/elemento sin definir: <{arg}>"
        )
    ...
    def menu(self) -> None:
        ...
        new_usr = Button(
            ...
            fg="black",
            font=("Calibri", "14", "bold"),
            command=lambda: self.go("home -> register"),
        ).grid(row=1, column=0, sticky="ew")
        # remove a user
        del_usr = Button(
            self.welcome,
            ...
            fg="black",
            font=("Calibri", "14", "bold"),
            command=lambda: self.go("home -> del_user"),
        ...
        view_usrs = Button(
            ...
            fg="black",
            font=("Calibri", "14", "bold"),
            command=lambda: self.go("view users"),
        ...
        view_report = Button(
            ...
            fg="black",
            font=("Calibri", "14", "bold"),
            command=lambda: self.go("view registry"),
        ).grid(row=4, column=0, sticky="ew")
        # docs button
        view_docs = Button(
            self.welcome,
            text="Ver el instructivo en linea",
            bg="whitesmoke",
            fg="black",
            font=("Calibri", "14", "bold"),
            command=lambda: startfile(
                "https://controldeagua.github.io/ControlDeAgua-docs"
            ),
        ).grid(row=5, column=0, sticky="ew")
        # manage the product info
        manage_product = Button(
            self.welcome,
            text="Manejar la informacion de producto",
            bg="whitesmoke",
            fg="black",
            font=("Calibri", "14", "bold"),
            command=lambda: self.go("manage product info"),
        ).grid(row=6, column=0, sticky="ew")
        # exit button
        get_out = Button(
            self.welcome,
            text="Salir de la pagina",
            bg="red",
            fg="white",
            font=("Calibri", "14", "bold"),
            command=self.root.quit,
    ...
    def del_user_internal(self) -> None:
        ...
        if not messagebox.askyesno(
            "¿Proceder?",
            """¿En verdad desea proceder con esta accion?
...
pero los registros a su nombre no se eliminan)""",
        ...
        try:
            ...
            if len(selected_usr) < 1:
                raise ValueError('Expected non-empty strings, got ""')
            # update
            to_save = {}
            with open(find_our_file("tools/users.json"), "r") as f:
                f_parsed = json.loads(f.read())
                if selected_usr not in f_parsed:
            ...
            messagebox.showerror(
                "Error al registrar",
                f"""Ha sucedido un error al retirar al usuario. Puede que
...
(Error: '{str(e)}')""",
        ...
        messagebox.showinfo(
            ...
            f"""Se ha eliminado con exito al usuario
'{selected_usr}' del registro de usuarios.""",
    ...
    def del_user_page(self) -> None:
        ...
        name_l = Label(
            self.del_page,
            text="Introduzca nombre completo (sin acentos):",
        ...
        cancel = Button(
            ...
            bg="gray",
            fg="white",
            command=lambda: self.go("del_user -> home"),
        ).grid(row=1, column=0, sticky="ew")
        go_ahead = Button(
            self.del_page,
            text="Eliminar",
            ...
            bg="red",
            fg="white",
            command=lambda: self.del_user_internal,
        ).grid(row=1, column=1, sticky="ew")
    ...
    def reg_page(self) -> None:
        ...
        name_l = Label(
            self.regframe,
            text="Introduzca una contraseña para relacionar con la cuenta:",
            font=("Calibri", "13", "bold"),
            bg="whitesmoke",
            fg="black",
        ).grid(row=1, column=0, sticky="ew")
        ...
        skip = Button(
            ...
            bg="red",
            fg="white",
            command=lambda: self.go("register -> home"),
        ).grid(row=2, column=0, sticky="ew")
        send_b = Button(
            self.regframe,
            text="Registrar",
            ...
            bg="green",
            fg="white",
            command=self.register_internal,
    ...
    def see_registry_internal(self) -> None:
        ...
        try:
            date_a, date_b = (
                self.date_list[self.di1.get() - 1],
                self.date_list[self.di2.get() - 1],
            )
            compare_dates(date_a, date_b)
        except (KeyError, NameError, ValueError) as exc:
            messagebox.showerror(
                "Error al mostrar",
                f"""No se pudo mostrar los datos solicitados. El sistema no pudo hallar
...
(Error: '{str(exc)}')""",
        ...
        CUR.execute(
            "SELECT vendor_id, product_id, odometer_read, cost, datetime, noentry_reason FROM Prompt WHERE datetime >= ( ? ) AND datetime <= ( ? )",
            (date_a, date_b),
        )
        finale = CUR.fetchall()
        ...
        for v, p, o, c, d, r in finale:
            ...
            finale_str += add_on
        intro = (
            f"VENTAS DESDE '{date_a}' HASTA '{date_b}'\n"
            + "=" * 60
            + "\n- Vendedores: "
        )
    ...
    def see_registry(self) -> None:
        ...
        first_date_l = Label(
            self.sales_f,
            text="Introduzca una fecha inicial para buscar:",
            bg="whitesmoke",
            fg="black",
            font=("Calibri", "13", "bold"),
        ).grid(row=0, column=0, sticky="ew")
        ...
        self.di1 = IntVar()
        first_date_m = get_menubutton(
            self.sales_f, self.date_list, self.di1, 0, 1, "ew"
        )
        # second date
        scnd_date_l = Label(
            self.sales_f,
            text="Introduzca una fecha final para buscar:",
            bg="whitesmoke",
            fg="black",
            font=("Calibri", "13", "bold"),
        ).grid(row=1, column=0, sticky="ew")
        ...
        cancel_b = Button(
            ...
            fg="white",
            font=("Calibri", "13", "bold"),
            command=lambda: self.go("registry_view -> home"),
        ).grid(row=2, column=0, sticky="ew")
        send_b = Button(
            self.sales_f,
            text="Buscar",
            ...
            fg="white",
            font=("Calibri", "13", "bold"),
            command=self.see_registry_internal,
        ).grid(row=2, column=1, sticky="ew")
    ...
    def register_internal(self) -> None:
        ...
        try:
            ...
            if len(new_usr) < 1:
                raise ValueError(f'Expected non-empty strings, got ""')
            # update
            to_save = []
            with open(find_our_file("tools/users.json"), "r") as f:
            ...
            messagebox.showerror(
                "Error al registrar",
                f"""Ha sucedido un error al registrar al usuario. Puede que
haya dejado la entrada totalmente vacia o haya agregado algun caracter no
permitido. Verifique e intente de nuevo.
...
(Error: '{str(e)}')""",
        ...
        messagebox.showinfo(
            "Proceso completado",
            f"""El proceso se ha completado exitosamente. Se le redirigira a la pagina de inicio.

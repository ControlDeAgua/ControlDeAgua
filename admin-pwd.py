...
def main() -> None:
    try:
        ...
        if not len(open(find_our_file("tools/admin.txt")).read().strip()) < 1:
            ...
            print(
                Fore.YELLOW
                + "ADVERTENCIA: No hay contraseña previa. Esto no afecta el proceso, solo que no se le va a pedir la contraseña previa."
            )
        ...
        print(
            Fore.RED + f"Error: No se pudo decodificar un caracter ({str(exc)}).",
            end=" ",
        )
    except OSError:
        print(
            Fore.RED + f"Error: El sistema rechazo alguna operacion ({str(exc)}).",
            end=" ",
        )
        ...
        print(
            Style.BRIGHT
            + Fore.RED
            + f"""Error fatal: Hubo un error no esperado. Por favor reportelo a <https://github.com/ControlDeAgua/bug_tracker>.

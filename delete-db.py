    ...
    try:
        if not os.path.exists(pathname):
            messagebox.showerror(
                "Error al eliminar",
                f"""Parece que no se puede eliminar '{pathname}' porque este archivo no existe.
¿No ha eliminado previamente esta base de datos? Verifique o intente de nuevo.""",
        ...
        messagebox.showinfo(
            "Proceso terminado",
            "El proceso ha terminado exitosamente. La base de datos fue eliminada.",
        )
    except Exception as e:
        messagebox.showerror(
            "Error al eliminar",
            f"""Parece que no se puede eliminar el archivo '{pathname}'.
